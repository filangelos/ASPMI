...
%% Periodogram against "alpha" & Resolution Threshold
...
for i = 1:length(alpha)
    ...
    if mod(alpha(i) * 100, 10) == 0
        ...
        fig = figure("Name", sprintf("Half Periodogram %.1f", alpha(i)));
        plot(f, Pxx_half);
        title(sprintf("\\textbf{Rectangular} Window: Periodogram with $\\alpha=%.1f$", alpha(i)));
        xlabel("Normalised Frequency ($\frac{\pi\ rad}{sample}$)")
        ylabel("Power Density (dB)")
        axis([0.35 0.45 -60 25]);
        grid on; grid minor;
        saveas(fig, sprintf("Assignment1/assets/1.3/b/periodogram-example-alpha_%.1f.eps", alpha(i)), "epsc");
    end
    % find sidelobes peaks & sort them
    peaks = sort(findpeaks(mag2db(Pxx_half)), 'descend');
...
plot(alpha, recover);
title("\textbf{Rectangular} Window: Resolution Threshold and $\alpha$");
xlabel("$\alpha$");
...
grid on; grid minor;
saveas(fig, "Assignment1/assets/1.3/b/periodogram-resolution-threshold-alpha.eps", "epsc");
